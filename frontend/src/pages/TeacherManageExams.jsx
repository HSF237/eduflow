...
export default function TeacherManageExams() {
  ...
  const fetchSubjectsForClass = async (classId) => {
    if (!classId || !teacher) { setAvailableSubjects([]); return; }
    try {
      const subjs = await getSubjects(teacher.school_id, classId);
      setAvailableSubjects(subjs);
    } catch (err) { setAvailableSubjects([]); }
  };

  const handleClassChange = (classId) => {
    setFormData(prev => ({ ...prev, class_id: classId, subject: '' }));
    setSelectedSubjects([]);
    ...
    try {
      if (editingExam) {
        ...
        if (subsToCreate.length === 0) {
          showToast('No subjects available or selected for this class', 'error');
          setSaving(false);
          return;
        }
        ...
        showToast(`Created ${subsToCreate.length} exam(s) successfully`);
      } else {
        await createExam({
          name: formData.name,
          type: formData.type,
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              {!bulkMode ? (
                ...
                <div className="space-y-3">
                  ...
                  {availableSubjects.length > 0 ? (
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Select Subjects (leave empty for ALL):</label>
                    ...
                    <p className="text-xs text-amber-600 bg-amber-50 p-2.5 rounded-lg border border-amber-200">
                      No subjects configured for this class yet. Add subjects from the Subjects sidebar link first.
                    </p>
                  )}
                </div>
              )}
