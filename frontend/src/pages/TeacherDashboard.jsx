  ...
  const load = async (user) => {
    try {
      ...
      if (teacherData.school_id) {
        allClasses = await getClasses(teacherData.school_id).catch(() => []);
      }
      ...
      if (myClasses.length === 0 && teacherData.assigned_classes && teacherData.assigned_classes.length > 0) {
        myClasses = allClasses.filter((c) => teacherData.assigned_classes.includes(c.id));
      }
      ...
      <div className="min-h-screen bg-slate-50 font-sans">
        ...
        <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            <button
              onClick={() => navigate(createPageUrl('JoinSchool'))}
              className="border border-slate-300 hover:bg-slate-100 text-slate-700 font-semibold px-6 py-3 rounded-xl transition-colors flex items-center gap-2"
            >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="max-w-5xl w-full mx-auto px-4 py-6 space-y-6">
          ...
          {classes.length > 0 && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <select
                ...
                className="w-full sm:w-auto border border-slate-300 rounded-lg px-3 py-2.5 sm:py-2 text-sm font-medium bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
                {classes.map((c) => (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {/* Students */}
            <div className="bg-emerald-500 text-white rounded-2xl p-5 flex flex-col gap-1 shadow-sm">
